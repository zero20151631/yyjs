/*! ykv <0.1.4@2016-11-10T08:51Z> | Copyright (c) 2015-2016 1VERGE, Inc */
!
function(t, e) {
...
function() {
			...
			u = a.extend({
				...
				quality: function(t) {
					...
					if (arguments.length > 0) if ("PLAYING" == this.currentState || "ERROR" == this.currentState) {
						if (t != this.currentQuality) {
							var n = e.playData.getM3u8src(e.currentLang.vid, t, e._options.password);
							if (n) {
								e.currentQuality = t;
								var i = e.currentTime();
								e.src(n),
				...
				rePlay: function() {
					var t = this;
					if ("INIT" != this.currentState && "AD" != this.currentState) return r.browser.isIPAD && t.src(t.playData.getM3u8src(t.currentLang.vid, t.currentQuality, t._options.password)),
					t.seek(0),
					this
				},
				getState: function() {
				...
				play: function() {
					var t = this;
					null != t.src() && "" != t.src() || t.src(t.playData.getM3u8src(t.currentLang.vid, t.currentQuality, t._options.password)),
					t._setState("PLAYING"),
					t.video.play()
				},
				_initPlay: function() {
					...
					t._options.autoplay && (t.src(t.playData.getM3u8src(t.currentLang.vid, t.currentQuality, t._options.password)), t.play(), t._setState("PLAYING"), t._super())
				},
				bindEvents: function() {
					var t = this,
					e = this._super;
	...
	function(t, e, n) { (function(e, i, r, o) {
			function a(t) {
				var n = null,
				i = t;
			...
			var d = e.VideoPlayer.extend({
				...
				_setState: function(t) {
					...
					if (e) {
						...
						if (i[t]) try {
							n.trigger(i[t], s.call(arguments, 1))
						} catch(o) {
				...
				_getPlayData: function(t) {
					...
					function() {
						this._retryPlay < 3 ? (n(t), this._retryPlay++) : (e._setError(2003, "Get PlayData Failed"), delete this._retryPlay)
					})
				},
				_parseData: function(t, e) {
					...
					n.videoData = t,
					n.playData = i,
					i.error ? n._setError(i.error.code, i.error.msg) : (n._setVideoData(r.defaultLanguage), e())
				},
				_setVideoData: function(t) {
					var e = this,
					n = e.playData.getIndexes();
					if (n.length <= 0) return void e._setError(2002, "Parse Data Error");
					var i = e.playData.getIndexes(t) || n[0];
					e.currentLang = i;
					var o = e._getTypeByQuality(this._options.quality || r.defaultQuality);
					if (!o) return void e._setError(2002, "æ²¡æœ‰å¯¹åº”çš„æ ¼å¼");
	...
	function(t, e, n) { (function(e, n, i) {
			t.exports = e.ObjectCreate({
				...
				getPlayData: function(t) {
					var n = this;
					return new e.Promise(function(r, o) {
						var a = {
							vid: t.vid,
							ct: i.ctype
						};
						t.client_id && (a.cid = t.client_id),
						t.sign && (a.sign = t.sign),
						t.password && (a.pwd = escape(t.password)),
						e.jsonp({
							url: "http://play.youku.com/partner/get.json",
							callback: "callback",
							data: a,
							time: 5e3,
							success: function(e) {
								var i = e.data;
								i.stream || i.error ? (n._Cache[t.vid] = i, r(i)) : o({
									code: "101",
									note: "è¯¥è§†é¢‘æš‚ä¸èƒ½æ’­æ”¾"
								})
							},
							fail: function(t) {
								o({
	...
	function(t, e, n) { (function(e, n, i, r, o) {
			var a = e.CoreObject.extend({
				...
				_initSecurity: function(t) {
					if (!t.security.encrypt_string || !t.security.ip) return this.setError(2004, "Parse Data Error"),
					!1;
				...
				_parseData: function(t) {
					...
					i = 0; i < t.length; i++) {
						var r = t[i],
						o = e.getTypeByKey(r.stream_type),
						a = e.getLangByKey(r.audio_lang);
						if (o && a) {
							for (var s = r.segs,
							u = 0,
							l = 0,
							c = [], d = 0; d < s.length; d++) {
								...
								var p = {
									no: d,
									seconds: Number(h.total_milliseconds_video) / 1e3,
									seconds_video: Number(r.milliseconds_video) / 1e3,
									key: h.key,
									fileId: e._getFileId(r.stream_fileid, d),
									type: o
								};
								p.src = e._getVideoSrc(p),
								u += parseInt(p.seconds),
								l += parseInt(h.size),
								c.push(p)
							}
							if (! ("3gphd" == o.key && parseInt(u) > 7200)) {
								e._getLangVid(a.key);
								n[a.key] || (n[a.key] = {
									key: a.key,
									cn: a.value,
									id: a.id,
									vid: e._getLangVid(a.key),
									types: {}
								}),
								n[a.key].types[o.key] = {
									no: i,
									logoFlag: "none" != r.logo ? 1 : 0,
									type: o,
									segs: c,
									seconds: u,
									size: l
	...
	function(t, e, n) { (function(e, n, i, r, o) {
			...
			a.prototype = {
				...
				addPlayerDurationReport: function(t) {
					...
					o = {};
					o.viewUserId = 0,
					o.sid = this.player.playData.sid,
					o.videoid = this.vd.id,
					o.number = t;
					var a = new Date;
					o.rnd = (a.getTime() - this.player.abstarttime) / 1e3,
					o.showid_v2 = "",
					o.Copyright = "",
					o.Tid = "",
					o.hd = 0,
					o.ikuflag = "m",
					o.hd = this.player.data.type.hd,
					o.totalsec = this.player.data.totalTime,
					o.playComplete = 0,
					61 == t && (o.playComplete = 1),
					59 == t && (o.referUrl = (this.player._options.vvlogconfig || {}).rurl, o.url = encodeURIComponent(window.location.href), o.starttime = 0),
					o.currentPlayTime = parseInt(this.player.currentTime() || 0),
					o.continuationPlay = 0,
					o.pid = this.player._options.client_id,
					o.timestamp = (new Date).getTime(),
					o.ctype = "86",
					o.ev = "1",
					o.tk = this.player.playData.token,
					o.oip = r.security.ip,
					o.paystate = this.getPayState(),
					o.playstate = null == r.trial ? "1": "2";
					var s = "http://stat.youku.com/player/addPlayerDurationReport?";
					n.log.send(s + i.urlParameter(o))
				},
				addPlayerStaticReport: function() {
					var t = "http://stat.youku.com/player/addPlayerStaticReport",
					e = {},
					r = this.vd;
					e.videoid = this.vd.id,
					(r.token || this.player.playData.token) && (e.t = r.vv || ""),
					e.totalsec = parseInt(r.video.seconds),
					e.ikuflag = "m_" + this.getShowFlag(),
					e.url = encodeURIComponent(this.getParentUrl() ? this.getParentUrl() : window.location.href),
					e.fullflag = "0",
					e.source = "video",
					e.referer = (this.player._options.vvlogconfig || {}).rurl,
					e.sid = this.player.playData.sid;
					var o = i.urlParameter(e);
					n.log.send(t + "?" + o)
				},
